...
<body>
	...
	<div class="container">
		<form action="product?action=onedit" method="post"
			...
			<div class="form-group">
				<label for="name-inp"><b>Name</b></label> <input type="text"
					name="name" value="${ product.name }" id="name-inp"
			...
			<div class="form-group">
				<label for="category"><b>Category</b></label> <select
					...
					<c:forEach items="${ listCategory }" var="ct" varStatus="vs">
						<option value="${ ct.id }"
							${ product.category_id == ct.id ? "selected" : "" }>${ ct.name }</option>
					</c:forEach>
				</select>
			</div>
			<div class="form-group">
				<label for="price-inp"><b>Price</b></label> <input type="number"
					name="price" id="price-inp" value="${ product.price }"
					class="form-control rounded-0" placeholder="Price">
			</div>
			<div class="form-group">
				<label for="description"><b>Description</b></label>
				<textarea name="description" id="description"
					class="form-control rounded-0" cols="" rows="3">${ product.description }</textarea>
			</div>
			<div class="form-group">
				<label for="file-inp"><b>Thumbnail</b></label> <input type="file"
					name="file" id="file-inp" class="form-control rounded-0"> <img
					alt="" id="preview-img" src="http://localhost:8080/be/${ product.thumbnail }"
					width="400px">
			</div>
	...
	<script type="text/javascript">
		...
		fileInputElem.onchange = evt => {
			  ...
			  } else previewElem.src = valueImageDefault;
			}
		</script>
</body>
</html>